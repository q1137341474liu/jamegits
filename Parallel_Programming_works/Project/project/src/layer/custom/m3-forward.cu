                                  ...
                                  int Batch, int Map_out, int Channel, int Height, int Width, int K)
{
    ...
    int loop_unroll_factor = 4;
    int tile_number = (mask_unrolled_width - 1) / TILE_WIDTH + 1;
    int iteration = tile_number / loop_unroll_factor;
    ...
    for (int i = loop_unroll_factor * iteration; i < tile_number; i++){
        int col_m = i * TILE_WIDTH + tx;
        if ((row < mask_unrolled_height) && (col_m < mask_unrolled_width)){           
            tileB[ty][tx] = __float2half(mask[row * mask_unrolled_width + col_m]);
        } else { 
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    dim3 grid_unroll((Height_out * Width_out + TILE_WIDTH - 1) / TILE_WIDTH, (Map_out + TILE_WIDTH - 1) / TILE_WIDTH, Batch);
